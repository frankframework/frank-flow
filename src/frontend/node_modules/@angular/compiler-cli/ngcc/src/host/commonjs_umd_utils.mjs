 ...
 */
export function findNamespaceOfIdentifier(id) {
 ...
 * `var <id> = require('...')`
 */
export function findRequireCallReference(id, checker) {
    var _a, _b;
 ...
 */
export function isWildcardReexportStatement(stmt) {
 ...
 * ```
 */
export function isDefinePropertyReexportStatement(stmt) {
    if (!ts.isExpressionStatement(stmt) || !ts.isCallExpression(stmt.expression)) {
        return false;
    }
    ...
    return (propertyDescriptor.properties.some(prop => prop.name !== undefined && ts.isIdentifier(prop.name) && prop.name.text === 'get'));
}
/**
 ...
 */
export function extractGetterFnExpression(statement) {
    const args = statement.expression.arguments;
    const getterFn = args[2].properties.find(prop => prop.name !== undefined && ts.isIdentifier(prop.name) && prop.name.text === 'get');
 ...
 */
export function isExternalImport(path) {
    return !/^\.\.?(\/|$)/.test(path);
}
/**
 ...
 * `exports.<foo>`.
 */
export function isExportsDeclaration(expr) {
 ...
 * `exports.<foo> = <bar>`.
 */
export function isExportsAssignment(expr) {
 ...
 * `exports.<foo> = <bar>;`.
 */
export function isExportsStatement(stmt) {
